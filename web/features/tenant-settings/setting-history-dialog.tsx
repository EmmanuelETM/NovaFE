...
import { useTenantSettingHistory } from "./use-tenant-settings";

/** `null` = vuelta al default. */
function display(value: string | null, fallback: string): string {
  return value ?? fallback;
}

export function SettingHistoryDialog({ setting }: { setting: TenantSetting }) {
  const [open, setOpen] = useState(false);
  const { data, isPending, error } = useTenantSettingHistory(setting.key, open);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Cada cambio del valor, el más reciente primero.
          </DialogDescription>
        </DialogHeader>
