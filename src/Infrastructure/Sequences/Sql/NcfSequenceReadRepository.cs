    ...
    ICurrentTenant currentTenant) : INcfSequenceReadRepository
{
    ...
    public async Task<NcfSequenceDto?> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        var sql =
            $"""
            SELECT {Columns}
    ...
    public async Task<IReadOnlyList<NcfSequenceDto>> ListAsync(CancellationToken ct = default)
    {
        var sql =
            $"""
            SELECT {Columns}
