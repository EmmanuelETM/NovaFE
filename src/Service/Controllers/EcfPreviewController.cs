...
/// diffear, sin firmar ni enviar a la DGII. Cada respuesta trae el resultado de
...
public sealed class EcfPreviewController(
    ...
    IEcfXsdValidator validator) : ApiController
{
    ...
    /// es tipo 32); <c>?raw=true</c> devuelve el XML crudo en vez del JSON.
    /// </summary>
    [HttpGet("samples/{slug}")]
    public IActionResult Sample(string slug, [FromQuery] bool rfce = false, [FromQuery] bool raw = false)
    {
    ...
    /// <c>{ "type": 31, "lines": [{ "unitPrice": 1000 }] }</c>. <c>?raw=true</c>
    /// devuelve el XML crudo.
    /// </summary>
    [HttpPost]
    public IActionResult Preview([FromBody] EcfPreviewRequest request, [FromQuery] bool raw = false)
        => EcfPreviewMapper.ToDocument(request).Match(doc => Ecf(doc, raw), Problem);

    /// <summary>Genera el XML <c>&lt;RFCE&gt;</c> (el <c>document</c> debe ser tipo 32).</summary>
    [HttpPost("rfce")]
    public IActionResult PreviewRfce([FromBody] RfcePreviewRequest request, [FromQuery] bool raw = false)
        => EcfPreviewMapper.ToDocument(request.Document).Match(
            doc => Rfce(doc, request.SecurityCode, raw),
            Problem);
    ...
    private IActionResult Render(string xml, ErrorOr<Success> xsd, bool raw)
    {
        if (raw)
        {
            Response.Headers["X-Ecf-Xsd-Valid"] = xsd.IsError ? "false" : "true";
