...
**vendorizados y embebidos** en `src/Infrastructure/Ecf/Xsd/`. **El formato del e-CF
está completo**: los diez tipos (31–34, 41, 43–47) con todos los bloques
(InformacionesAdicionales, Transporte, OtraMoneda, Subtotales, DescuentosORecargos/
Sección D, Paginacion, desglose ImpuestosAdicionales, Mineria, TablaSubcantidad…) **y
el RFCE** (`RfceSerializer` / `IRfceSerializer` — resumen del tipo 32 &lt; DOP 250 k;
`EcfDocument.QualifiesForRfce`). Las variaciones por tipo del serializador viven en
`EcfXmlProfile` (tabla, Infrastructure); el emisor usa el mini-DSL `EcfElementWriter`.
Particularidades: 41/47 con `<Retencion>` obligatoria (47 solo ISR); 43 sin
`<Comprador>`; 43/44/47 solo exentas; 45 ≡ 31; 46 solo tasa 0 %. Bloques
transversales = **passthrough** (el cliente trae los montos), salvo la Sección D que
el motor reconcilia. No cambiar sin leer `docs/ecf-xml.md`. `EcfDocument` **no es** el
payload de la API (ese es curado — `docs/api-ecf.md`).
...
ambiente (`ICurrentTenant.Environment` — lo trae la API key; o el
`EmitterProfile.DefaultEnvironment` en el camino `X-Tenant-Id` de Development. El
payload **no** lleva `environment`) →
