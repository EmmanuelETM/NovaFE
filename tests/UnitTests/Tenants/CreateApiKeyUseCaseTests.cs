...
public class CreateApiKeyUseCaseTests : UseCaseTestBase
{
    ...
    {
        var result = await Sut().Execute(new CreateApiKeyCommand(TenantId, "ERP contable", null, null));
    ...
    {
        var result = await Sut().Execute(new CreateApiKeyCommand(TenantId, null, "Production", null));
    ...
    public async Task Fails_when_the_emitter_profile_is_missing()
    {
        ...
        var result = await Sut().Execute(new CreateApiKeyCommand(TenantId, null, null, null));
    ...
    {
        var result = await Sut().Execute(new CreateApiKeyCommand(TenantId, null, "Staging", null));
    ...
    public async Task Rejects_an_expiration_in_the_past()
    {
        var result = await Sut().Execute(
            new CreateApiKeyCommand(TenantId, null, null, Clock.GetUtcNow().AddDays(-1)));
